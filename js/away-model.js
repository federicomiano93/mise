...
// ⚠️ THIS FILE HAS NO IMPORTS, ON PURPOSE. It is copied byte-for-byte into
// functions/ — a deploy uploads only that folder, so `../js` would resolve here
// and be missing in the cloud — and pinned by tests/copie-allineate.test.mjs.
// Same shape, same reason, as push-model.js. A copy that must be EDITED to run on
// the server is a copy that drifts, which is the danger pinning exists to prevent.
...
// ⚠️ IT RETURNS '' RATHER THAN A WORD, AND THAT IS WHY THIS FILE HAS NO IMPORTS.
// A word would mean asking the dictionary, and the dictionary cannot come to the
// server. The SCREEN supplies the fallback; what matters here is only that a raw
// uid never reaches a person's eyes.
export function personName(member) {
  const full = [member?.firstName, member?.lastName]
    .map(p => String(p || '').trim()).filter(Boolean).join(' ');
  return full || String(member?.email || '').trim() || '';
}
