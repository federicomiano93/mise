...
// Plant the Orders data INSIDE a restaurant's folder — which is where the app
...
export async function seedAll(restaurant = 'main') {
  const at = path => `restaurants/${restaurant}/${path}`;
  const stamp = data => ({ ...data, bakery: restaurant });
...
// would stop half way: restaurants present, access documents missing — which on
...
export async function seedAccount(email, password, restaurants) {
  ...
  await seedDoc(`users/${body.localId}`, { restaurants });
  return body.localId;
}
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  // Two restaurants, so "each one sees only its own" can be checked by eye and
  // not only asserted in a test.
  await seedAll('main');
  await seedDoc('restaurants/main', { name: 'The Italian Club' });
  await seedDoc('restaurants/main/config/calculator',
    { bakery: 'main', configRev: 1, clients: [], recipes: [] });
  await seedDoc('restaurants/main/recipes/CAT_1',
    { bakery: 'main', name: 'Sourdough', ingredients: [] });

  await seedDoc('restaurants/trattoria-rosa', {
    name: 'Trattoria Rosa',
    sections: { orders: true, calculator: false, catalogue: false },
  });
  await seedDoc('restaurants/trattoria-rosa/suppliers/SUP_ROSA', {
    bakery: 'trattoria-rosa', name: 'Rosa Fresh Fish', category: 'Fish',
    deliveryDays: ['Monday'], orderDays: ['Sunday'], active: true,
  });
  await seedDoc('restaurants/trattoria-rosa/ingredients/ING_ROSA', {
  ...
  await seedAccount('club@club.test', PASSWORD, { main: true });
  await seedAccount('rosa@club.test', PASSWORD, { 'trattoria-rosa': true });
  await seedAccount('owner@club.test', PASSWORD, { main: true, 'trattoria-rosa': true });
  await seedAccount('nobody@club.test', PASSWORD, {});

  console.log(`Seeded the emulator:
  restaurants/main — The Italian Club, every section
  ...
  restaurants/trattoria-rosa — Orders only, its own supplier + ingredient

  Sign in with any of these (password: ${PASSWORD}):
    club@club.test    → The Italian Club
    rosa@club.test    → Trattoria Rosa (Orders only)
    owner@club.test   → both, so the restaurant picker appears
    nobody@club.test  → an account with no restaurant
`);
}
