...
test('a day that was never written reads as an empty list, not a crash', () => {
  assert.deepEqual(normalizeDay(null, 'Monday'), { day: 'Monday', items: [] });
  assert.deepEqual(normalizeDay(undefined, 'Friday'), { day: 'Friday', items: [] });
  assert.deepEqual(normalizeDay({}, 'Sunday'), { day: 'Sunday', items: [] });
});
...
test('the whole collection always comes back with all seven days', () => {
  for (const input of [[], null, 'nonsense', undefined, {}]) {
    ...
    WEEKDAYS.forEach(d => assert.deepEqual(days[d], []));
  }
});
...
test('a document whose id is not a weekday is ignored, not rendered', () => {
  ...
  assert.deepEqual(Object.keys(days), [...WEEKDAYS]);
  assert.deepEqual(days.Monday, [{ name: 'Cornetti', qty: 24 }]);
  assert.equal(days.Tuesday.length, 0);
});
